...
task Build-ConDep-Node -depends Clean-ConDep-Node, Init { 
	...
	cmd /c $pwd\lib\ilmerge\ILMerge.exe /wildcards /internalize /allowDup /out:$build_directory\$condep_node\ConDepNode.exe $build_directory\$condep_node\ConDep.Node.exe $build_directory\$condep_node\*.dll
